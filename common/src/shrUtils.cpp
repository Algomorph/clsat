...
// *********************************************************************
static int shrLogV(int iLogMode, int iErrNum, const char* cFormatString,
		va_list vaArgList) {
...
//              return -1;
	...
	for (pStr = cFormatString; *pStr; ++pStr) {
		...
		if (*pStr != '%') {
			...
			while (pStr
					&& ((sFormatChars.find(*pStr) != string::npos) || bRepeater)) {
				sFormatSpec += *pStr;
			...
			case '%':   // special handling for string of %%%%
			{
				...
				if (iLogMode & LOGFILE) {
					...
					if (iLogMode & MASTER) {
						szNumWritten = fprintf(pFileStream1,
								sFormatSpec.c_str());         // master log file
					}
				}
				continue;
			}
			...
			case 's':   // string of single byte chars
			{
				...
				if (iLogMode & LOGFILE) {
					szNumWritten = fprintf(pFileStream0, sFormatSpec.c_str(),
							cArg);       // sample log file
					if (iLogMode & MASTER) {
						szNumWritten = fprintf(pFileStream1,
								sFormatSpec.c_str(), cArg);   // master log file
					}
				}
				continue;
			}
			...
			case 'i':   // signed decimal integer
			{
				...
				if (iLogMode & LOGFILE) {
					szNumWritten = fprintf(pFileStream0, sFormatSpec.c_str(),
							iArg);       // sample log file
					if (iLogMode & MASTER) {
						szNumWritten = fprintf(pFileStream1,
								sFormatSpec.c_str(), iArg);   // master log file
					}
				}
				continue;
			}
			...
			case 'X':   // unsigned hexadecimal integer using "ABCDEF"
			{
				...
				if (iLogMode & LOGFILE) {
					szNumWritten = fprintf(pFileStream0, sFormatSpec.c_str(),
							uiArg);      // sample log file
					if (iLogMode & MASTER) {
						szNumWritten = fprintf(pFileStream1,
								sFormatSpec.c_str(), uiArg);  // master log file
					}
				}
				continue;
			}
			...
			case 'A':   // signed hexadecimal double precision float
			{
				...
				if (iLogMode & LOGFILE) {
					szNumWritten = fprintf(pFileStream0, sFormatSpec.c_str(),
							dArg);       // sample log file
					if (iLogMode & MASTER) {
						szNumWritten = fprintf(pFileStream1,
								sFormatSpec.c_str(), dArg);   // master log file
					}
				}
				continue;
			}
...
// Function to log standardized information to console, file or both
// *********************************************************************
int shrLogEx(int iLogMode = LOGCONSOLE, int iErrNum = 0,
...
//////////////////////////////////////////////////////////////////////////////
template<class T>
shrBOOL shrReadFile(const char* filename, T** data, unsigned int* len,
		bool verbose) {
	...
	if (!fh.eof()) {
		if (verbose)
			std::cerr
					<< "WARNING : readData() : reading file might have failed."
					<< std::endl;
	}
...
//////////////////////////////////////////////////////////////////////////////
template<class T>
shrBOOL shrWriteFile(const char* filename, const T* data, unsigned int len,
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFilef(const char* filename, float** data, unsigned int* len,
		bool verbose) {
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFiled(const char* filename, double** data, unsigned int* len,
		bool verbose) {
	return shrReadFile(filename, data, len, verbose);
}
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFilei(const char* filename, int** data, unsigned int* len,
		bool verbose) {
	return shrReadFile(filename, data, len, verbose);
}
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFileui(const char* filename, unsigned int** data,
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFileb(const char* filename, char** data, unsigned int* len,
		bool verbose) {
	return shrReadFile(filename, data, len, verbose);
}
...
//! @param len  number of data elements in data, -1 on error
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrReadFileub(const char* filename, unsigned char** data,
		unsigned int* len, bool verbose) {
	return shrReadFile(filename, data, len, verbose);
}
...
//! @param epsilon  epsilon for comparison
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrWriteFiled(const char* filename, const double* data,
		unsigned int len, const double epsilon, bool verbose) {
	return shrWriteFile(filename, data, len, epsilon, verbose);
}
...
//! @param epsilon  epsilon for comparison
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrWriteFilei(const char* filename, const int* data, unsigned int len,
		bool verbose) {
...
//! @param epsilon  epsilon for comparison
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrWriteFileui(const char* filename, const unsigned int* data,
		unsigned int len, bool verbose) {
	return shrWriteFile(filename, data, len, static_cast<unsigned int>(0),
			verbose);
}
...
//! @param epsilon  epsilon for comparison
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrWriteFileb(const char* filename, const char* data, unsigned int len,
		bool verbose) {
...
shrBOOL shrWriteFileub(const char* filename, const unsigned char* data,
		unsigned int len, bool verbose) {
	return shrWriteFile(filename, data, len, static_cast<unsigned char>(0),
			verbose);
}
...
//! @param epsilon  epsilon for comparison
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrWriteFileb(const char* filename, const unsigned char* data,
		unsigned int len, bool verbose) {
	return shrWriteFile(filename, data, len, static_cast<unsigned char>(0),
			verbose);
}
...
//////////////////////////////////////////////////////////////////////////////
shrBOOL loadPPM(const char* file, unsigned char** data, unsigned int *w,
	...
	if ((fgets(header, PGMHeaderSize, fp) == NULL) && ferror(fp)) {
		...
		std::cerr << "loadPPM() : File is not a valid PPM or PGM image"
				<< std::endl;
		*channels = 0;
		return shrFALSE;
	}
...
//! @param h     height of the image
//////////////////////////////////////////////////////////////////////////////
shrBOOL savePPM(const char* file, unsigned char *data, unsigned int w,
		unsigned int h, unsigned int channels) {
	ARGCHECK(NULL != data);
	ARGCHECK(w > 0);
	ARGCHECK(h > 0);
...
shrBOOL shrLoadPPM4ub(const char* file, unsigned char** OutData,
	...
	if (shrTRUE == bLoadOK) {
		...
		int size = *w * *h;
		if (*OutData == NULL) {
			*OutData = (unsigned char*) malloc(
					sizeof(unsigned char) * size * 4);
		}
...
//! @param h     height of the image
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrSavePPM4ub(const char* file, unsigned char *data, unsigned int w,
		unsigned int h) {
	// strip 4th component
	int size = w * h;
	unsigned char *ndata = (unsigned char*) malloc(
			sizeof(unsigned char) * size * 3);
...
//! @param h     height of the image
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrSavePGMub(const char* file, unsigned char *data, unsigned int w,
		unsigned int h) {
...
//! @param h     height of the image
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrLoadPGMub(const char* file, unsigned char** data, unsigned int *w,
		unsigned int *h) {
	unsigned int channels;
...
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrCheckCmdLineFlag(const int argc, const char** argv,
...
shrBOOL compareData(const T* reference, const T* data, const unsigned int len,
...
//////////////////////////////////////////////////////////////////////////////
template<class T, class S>
shrBOOL compareDataAsFloat(const T* reference, const T* data,
...
//////////////////////////////////////////////////////////////////////////////
template<class T, class S>
shrBOOL compareDataAsFloatThreshold(const T* reference, const T* data,
		const unsigned int len, const S epsilon, const float threshold) {
	ARGCHECK(epsilon >= 0);
...
shrBOOL shrComparePPM(const char *src_file, const char *ref_file,
	...
	if (shrLoadPPM4ub(ref_file, &ref_data, &ref_width, &ref_height)
			!= shrTRUE) {
		shrLog(
	...
	if (shrLoadPPM4ub(src_file, &src_data, &src_width, &src_height)
			!= shrTRUE) {
		shrLog(
	...
	// compare the images
	if (shrCompareubt(ref_data, src_data, src_width * src_height * 4, epsilon,
			threshold) == shrFALSE) {
		error_count = 1;
	}
...
//! @param threshold  threshold of pixels that can still mismatch to pass (i.e. 0.15f = 15% must pass)
////////////////////////////////////////////////////////////////////////////////
shrBOOL shrComparePGM(const char *src_file, const char *ref_file,
		const float epsilon, const float threshold) {
	unsigned char* src_data = NULL;
	unsigned char* ref_data = NULL;
	unsigned long error_count = 0;
